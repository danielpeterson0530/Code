...
def run_modules(): 
    ...
    cmds = ["nmcli device show",
            ...
            "resolvectl"]
    concat_output = ""
    dhreleasecmd = create_command("echo test2 | sudo -S dhclient -r")
    output = run_subprocess(dhreleasecmd)
    for cmd in cmds:
        processed_output = run_module(cmd)
...
def process_save_outputfile(output, devicenumber):
    ...
    def process_outputfilename(apnname, devicenumber, datacenter):
        ...
        outputinfo = "".join(("APN_", apnname, "_DC_", datacenter, "_Dev_", devicenumber, "_", currentdate))
        outputfilename = "".join((outputinfo, "_TestResults.txt",))
